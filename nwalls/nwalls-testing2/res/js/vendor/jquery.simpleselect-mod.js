	...
	var windowHeight = null,
		...
		positionAroundSsOption = function(ssOption) {
			...
			if (this.isScrollable) {
				...
				//this.ssOptionsContainer.height($(window).height() - this.ssOptionsContainer.offset().top - this.ssPlaceholderHeight);
			//	var ssOptionsContainerNewHeight = $(window).height() - this.ssOptionsContainer.offset().top - this.ssPlaceholderHeight;// - excessSpaceAbove - excessSpaceBelow;
				//console.log(ssOptionsContainerNewHeight );
				this.ssOptionsContainer
					.css({
						//top: - (ssOptionPosition.top - excessSpaceAbove)
						top: '100%'
					})
					.height($(window).height() - this.ssOptionsContainer.offset().top - this.ssPlaceholderHeight) // Using .height() instead of .css("height", value) gives us access to the cssHook for "height" that adapts the height value depending on the options container's box model (updated cssHook available since jQuery 1.8)
