	...
	var windowHeight = null,
		...
		init = function(options) {
			...
			options = $.extend({}, {
				...
				forceDown: (options && options.forceDown) || false
			}, options);
		...
		positionAroundSsOption = function(ssOption) {
			...
			var computePositioningValues = $.proxy(function() {
				...
				if (this.options.forceDown == true){
					freeVisibleSpaceBelow = (shouldDisplayContainerInsideWindow? windowHeight : documentHeight) - freeVisibleSpaceAbove - this.ssPlaceholderOffset.top - 2 * this.options.containerMargin;
				}
				else{
			...
			if (this.isScrollable) {
				...
				if (this.options.forceDown == true){
					this.ssOptionsContainer
						...
						.height($(window).height() - this.ssOptionsContainer.offset().top - this.ssPlaceholderHeight)
